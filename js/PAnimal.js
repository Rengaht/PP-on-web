function PAnimal(x_,y_){
	...
	this._skateboard=new SkateBoard(0,this._body_hei+this._leg_hei/4*3,this._wid*random(.5,.8),this._hei*random(.1,.4));
	this._firewheel=[];
	this._firewheel.push(new FireWheel(this._wid/4,-this._hei,this._hei*random(.3,.5)));
	this._firewheel.push(new FireWheel(this._wid/4*3,-this._hei,this._hei*random(.3,.5)));


	this._cur_mode;
	this._dest_mode;
	this._anim_mode_change=new FrameAnimation(random(40,80),random(0,80));
	this._src_ang=0;
	this._dest_ang=0;
...
PAnimal.prototype.Update=function(){
	...
	if(this._cur_mode==this._dest_mode){
		...
		// this._y+=(this._move_vel.y)*abs(sin(a)*.3+.8)*sp1_;	
	}else{
...
PAnimal.prototype.Draw=function(draw_fill){
		...
		var cur_body_wid=(this._cur_mode==0)?this._body_wid*elastic_portion:this._body_wid;
		...
		if(this._dest_mode!=this._cur_mode){
			...
			switch(this._dest_mode){
				case 1:
					this._skateboard.Draw(draw_fill,-width*(1-t),0);
					break;
				case 2:
		...
		bezierVertex(cur_body_wid+this._head_wid/2,-this._body_hei*.1*draw_portion,
					 				cur_body_wid+this._head_wid,this._head_hei+this._body_hei*.4*draw_portion,
		...
		for(var i=0;i<this._mleg;++i){
			...
			if(this._cur_mode==1) this.drawSkateLeg(cur_body_wid*(1.0-(i+this._leg_span[i])/this._mleg),this._body_hei,this._leg_wid,this._leg_hei,i);
		...
		if(DDEBUG){
			text(this._cur_mode+'-'+this._dest_mode,0,0);
		}
